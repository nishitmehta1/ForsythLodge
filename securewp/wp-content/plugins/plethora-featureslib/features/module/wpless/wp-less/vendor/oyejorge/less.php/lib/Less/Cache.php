...
class Less_Cache{
	...
	public static function Get( $less_files, $parser_options = array(), $modify_vars = array() ){
		...
		if( !empty($modify_vars) ){
			...
			if( !file_exists($vars_file) ){
				Plethora_WP::write_to_file($vars_file, $lessvars);
			}
		...
		$list[] = $compiled_name;
		$cache = implode("\n",$list);
		Plethora_WP::write_to_file( $list_file, $cache );


		//save the css
		Plethora_WP::write_to_file( $output_file, $compiled );
	...
	static function ListFiles($list_file, &$list, &$css_file_name ){
		...
		$list = explode("\n",Plethora_WP::get_file_contents($list_file));
