...
class Less_Parser{
	...
	private function GetRules( $file_path ){
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				switch(Less_Parser::$options['cache_method']){
					...
					case 'serialize':
						$cache = unserialize(Plethora_WP::get_file_contents($cache_file));
						if( $cache ){
							touch($cache_file);
							$this->UnsetInput();
							return $cache;
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				switch(Less_Parser::$options['cache_method']){
					case 'serialize':
						Plethora_WP::write_to_file( $cache_file, serialize($rules) );
					break;
					case 'php':
						Plethora_WP::write_to_file( $cache_file, '<?php return '.self::ArgString($rules).'; ?>' );
					break;
					case 'var_export':
						//Requires __set_state()
						Plethora_WP::write_to_file( $cache_file, '<?php return '.var_export($rules,true).'; ?>' );
					break;
				}
	...
	public function SetInput( $file_path ){
		...
		if( $file_path ){
			$this->input = Plethora_WP::get_file_contents( $file_path );
		}
