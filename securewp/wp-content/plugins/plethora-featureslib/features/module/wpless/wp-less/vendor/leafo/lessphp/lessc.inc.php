...
class lessc {
	...
	protected function tryImport($importPath, $parentBlock, $out) {
		...
		$root = $parser->parse(Plethora_WP::get_file_contents($realPath));
		...
		if($fullpath && ($fsize = filesize($fullpath)) !== false) {
			...
			if($fsize/1024 < 32) {
				...
				if(!is_null($mime)) // fallback if the mime type is still unknown
					$url = sprintf('data:%s;base64,%s', $mime, base64_encode(Plethora_WP::get_file_contents($fullpath)));
			}
		}
	...
	public function compileFile($fname, $outFname = null) {
		...
		$out = $this->compile(Plethora_WP::get_file_contents($fname), $fname);

		$this->importDir = $oldImport;

		if ($outFname !== null) {
			return Plethora_WP::write_to_file($outFname, $out);
		}
